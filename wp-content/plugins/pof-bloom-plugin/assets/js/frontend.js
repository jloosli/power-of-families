(function () {
	jQuery(document).ready(function ($) {
		...
		instructions
			.wrapInner("<div class='ins_content'></div>")
			.prepend(
				"<div class='ins_title'><h3>Instructions</h3> <small>(Click to expand)</small><small style='display:none;'>(Click to collapse)</small></div>"
			)
			.click(function () {
				'use strict';
		...
		$('form.assessment_create').submit(function (event) {
			...
			radios.each(function (idx, item) {
				if (groups.indexOf(item.name) === -1) {
					...
					if (
						!_.find(assessment, function (result) {
				...
				$('#missing_warning')
					.text(
						"It looks like you're missing " +
							bad.length +
							' answers. Please check your answers try submitting again.'
					)
					.show();
			} else {
		...
		$('form.goals_set', bloom)
			.find('.subcategories select')
			.change(function () {
				'use strict';
				var self = $(this),
					li,
									...
									$('<div></div>', {
										...
										click: function () {
											var me = $(this),
												...
												goal_id = fieldset.find(
													'.goal input[name^="suggestions"]'
												);
		...
		if (window.location.search.indexOf('page=goals.set') && window.google) {
			...
			function drawCharts() {
				for (var key in window.theCharts) {
					...
					var chart_locations = document.getElementsByClassName(
						window.theCharts[key].location
					);
